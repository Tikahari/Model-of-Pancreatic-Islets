...
def BAD_model_metrics_table(cell_rec_dict, simulation_time, time_step, df_path):
    ...
    total_spikes = [round(value, 2) for value in total_spikes]
    spike_frequency = [round(value, 2) for value in spike_frequency]
    max_value = [round(value, 2) for value in max_value]
    avg_spike_max = [round(value, 2) for value in avg_spike_max]
    min_value = [round(value, 2) for value in min_value]
    avg_spike_min = [round(value, 2) for value in avg_spike_min]
...
if __name__ == '__main__':
    # output file path
    simulation_setup = "BAD_figs_3-5_11mM_Glucose_200_seconds"
    simulation_output = "Data/" + simulation_setup + ".pkl"
    alpha_plot_path = 'Plots/' + simulation_setup + '_alpha.png'
    beta_plot_path = 'Plots/' + simulation_setup + '_beta.png'
    delta_plot_path = 'Plots/' + simulation_setup + '_delta.png'
    df_path = 'Plots/' + simulation_setup + '_metrics.png' 
    ...
    print("Create plots and dataframe")
