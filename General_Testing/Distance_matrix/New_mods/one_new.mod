NEURON{
    ...
    : New variables to track non-native hormones
    RANGE Sa, Ia, Sb, Gb, Id 
}
...
INITIAL{
    ...
    : New variables to track non-native hormones
    : Initialize to 0 for placeholders.
    Sa = 0
    Ia = 0
    Sb = 0
    Gb = 0
...
BREAKPOINT{
    ...
    EffId = 0.8/(1+exp(-(Id-1500)/500))
    iGABA = gGABAbar*EffId*(vd-vGABA)

    : Beta inhibiting alpha
    : Replaced I with Ia
    EffIa=(1-knockoutba)*((0.015/(1+exp((-Ia+1500)/200)))+ka1)+knockoutba*ka1
    ...
    EffSa=1/(1+exp(-(Sa-sombara2)/ssom2))
    iGIRKa=gGIRKbara*EffSa*(va-vGIRK)
    rm2a=(1-knockoutda)*ra/(1+exp(-(Sa-sombara)/ssom))+knockoutda*rako

    : Alpha stimulating beta
    : Replaced G with Gb
    r20=0.004/(1+exp(-Gb+gthresh))+0.006

    : Delta inhibiting beta
    : Replaced S with Sb
    gGIRKbarb=(1-knockoutdb)*10+knockoutdb*0
    EffSb=1/(1+exp(-(Sb-sombarb2)/10))
    iGIRKb=gGIRKbarb*EffSb*(vb-vGIRK)
    rm2b=(1-knockoutdb)*rb/(1+exp(-(Sb-sombarb)/ssomb))+knockoutdb*0.001
    ...
    : New variables to track non-native hormones
    : Sa = function of distance matrix of delta cells from alpha cells and JSS matrix
    : Ia = function of distance matrix of beta cells from alpha cells and JIS matrix
    : Sb = function of distance matrix of delta cells from beta cells and JSS matrix
    : Gb = function of distance matrix of beta cells from alpha cells and JGS matrix
    : Id = function of distance matrix of delta cells from betaa cells and JIS matrix
}
