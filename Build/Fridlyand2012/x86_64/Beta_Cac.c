...
#define nrn_init _nrn_init__Cac
#define _nrn_initial _nrn_initial__Cac
#define nrn_cur _nrn_cur__Cac
#define _nrn_current _nrn_current__Cac
#define nrn_jacob _nrn_jacob__Cac
#define nrn_state _nrn_state__Cac
#define _net_receive _net_receive__Cac 
#define states states__Cac 
...
#define DCaci _p[9]
#define IntCai _p[10]
#define DIntCai _p[11]
#define v _p[12]
#define _g _p[13]
#define _ion_Caci	*_ppvar[0]._pval
#define _style_Cac	*((int*)_ppvar[1]._pvoid)
#define _ion_IntCai	*_ppvar[2]._pval
#define _style_IntCa	*((int*)_ppvar[3]._pvoid)
#define _ion_iCaL	*_ppvar[4]._pval
#define _ion_iCaT	*_ppvar[5]._pval
#define _ion_iCaP	*_ppvar[6]._pval
...
static void nrn_alloc(Prop* _prop) {
 ...
 nrn_check_conc_write(_prop, prop_ion, 1);
 nrn_promote(prop_ion, 3, 0);
 	_ppvar[2]._pval = &prop_ion->param[1]; /* IntCai */
 	_ppvar[3]._pvoid = (void*)(&(prop_ion->dparam[0]._i)); /* iontype for IntCa */
 prop_ion = need_memb(_CaL_sym);
 	_ppvar[4]._pval = &prop_ion->param[3]; /* iCaL */
 prop_ion = need_memb(_CaT_sym);
 	_ppvar[5]._pval = &prop_ion->param[3]; /* iCaT */
 prop_ion = need_memb(_CaP_sym);
 	_ppvar[6]._pval = &prop_ion->param[3]; /* iCaP */
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 8, "cvodeieq");
 	...
 	ivoc_help("help ?1 Cac /ufrc/lamb/tikaharikhanal/Model-of-Pancreatic-Islets/Build/Fridlyand2012/x86_64/Beta_Cac.mod\n");
...
/*CVODE*/
 static int _ode_spec1 (double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) {int _reset = 0; {
   DCaci = ( ( fi * ( - iCaL - iCaP - iCaT - ( 2.0 * iPCa ) ) / ( 2.0 * F * Vi ) ) - ( ksg * Caci ) ) ;
   DIntCai = ( 0.001 * Caci ) ;
   }
 return _reset;
}
 ...
 static int states (double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) { {
    Caci = Caci + (1. - exp(dt*(( ( - ( ( ksg )*( 1.0 ) ) ) ))))*(- ( ( ( ( ( fi )*( ( - iCaL - iCaP - iCaT - ( ( 2.0 )*( iPCa ) ) ) ) ) / ( 2.0 * F * Vi ) ) ) ) / ( ( ( - ( ( ksg )*( 1.0 ) ) ) ) ) - Caci) ;
...
static void _ode_matsol(_NrnThread* _nt, _Memb_list* _ml, int _type) {
 ...
 static void _update_ion_pointer(Datum* _ppvar) {
   ...
   nrn_update_ion_pointer(_IntCa_sym, _ppvar, 2, 1);
   nrn_update_ion_pointer(_CaL_sym, _ppvar, 4, 3);
   nrn_update_ion_pointer(_CaT_sym, _ppvar, 5, 3);
   nrn_update_ion_pointer(_CaP_sym, _ppvar, 6, 3);
...
#if NMODL_TEXT
static const char* nmodl_filename = "/ufrc/lamb/tikaharikhanal/Model-of-Pancreatic-Islets/Build/Fridlyand2012/Beta_Cac.mod";
static const char* nmodl_file_text = 
  "NEURON{\n"
  "SUFFIX Cac \n"
