...
# It then grabs the variable you specify for each cell type and creates a tidy tibble with it.

grab_vars_of_interest <- function(data, var, var_yaml) {
  if (var == "current") {
    data %>%
      # Find cleaner way to name variables. This gets rid of point process version of currents
      ...
      select(contains(var_yaml$Current, ignore.case = F),
             # ideally find way to merge all time columns together so this isn't necessary
             # Time...1 is specified because when combining all individual output csv's into one tibble
             # the Time column repeats.
             Time...1) %>% 
      rename(Time = Time...1) %>% 
      pivot_longer(-Time,
                   names_to = "Temp",
      ...
      separate(Temp,
               ...
               sep = "_") %>% 
               # Current name is repeated in variable name so we only need to keep one
      select(-Current.) 
  } else if (var == "voltage") {
    data %>% 
    ...
    data %>% 
      ...
      separate(Temp,
               ...
               sep = "_") %>% 
               # create new column specifying intracellular or extracellular. Whenever an entry in the Hormone column
               # ends in "o" it will have the value Extracellular in this new column. Otherwise Intracellular
